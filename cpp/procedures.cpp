...
{
	pqxx::result r {txn.exec("SELECT * FROM get_author_name_like(" + std::to_string(name) + ")")};
	return r;
}
pqxx::result get_author_name_like(pqxx::work &txn, std::string name, int age)
{
	pqxx::result r {txn.exec("SELECT * FROM get_author_name_like(" + std::to_string(name) + ", " + std::to_string(age) + ")")};
	return r;
}